...
internal class AppworkshopScanner(SteamClient steamClient) : IScanner
{
    public IEnumerable<LocalResult> GetProfiles()
    {
        if (steamClient.Steam is null) yield break;

        foreach (var file in steamClient.Steam.GetSteamLibraries().Select(x => x.GetSteamappsDirectory())
